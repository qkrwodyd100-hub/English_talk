...
describe('converted 75-day curriculum', () => {
  it('contains the complete source workbook sequence with the app sentence contract', () => {
    ...
    expect(builtInSentences).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ id: 'day-75-10', english: 'I finished seventy five days and I speak with courage!', day: 75, source: 'builtIn' }),
    ]))
    ...
    expect([...new Set(builtInSentences.map((sentence) => sentence.day))]).toEqual(Array.from({ length: 75 }, (_, index) => index + 1))
    expect(builtInSentences.reduce<Record<number, number>>((counts, sentence) => ({ ...counts, [sentence.day]: (counts[sentence.day] ?? 0) + 1 }), {})).toEqual(
      Object.fromEntries(Array.from({ length: 75 }, (_, index) => [index + 1, 10])),
    )
  })

  it('has stable identifiers and exactly ten sentences for every day', () => {
    expect(new Set(builtInSentences.map(({ id }) => id)).size).toBe(750)
    expect(builtInSentences.map(({ id }) => id)).toEqual(
      Array.from({ length: 75 }, (_, dayIndex) =>
  ...
  it('provides one realistic two-to-four-turn mini dialogue for every day', () => {
    expect(builtInDialogues.map(({ day }) => day)).toEqual(Array.from({ length: 75 }, (_, index) => index + 1))
