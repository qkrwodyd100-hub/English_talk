...
import { MAX_COURSE_DAY, type Sentence } from './learning'
import { createListeningController, createListeningPlaylist, LISTENING_RATE_MAX, LISTENING_RATE_MIN, LISTENING_STORAGE_KEY, parseListeningPreferences, type ListeningPlayback, type ListeningPreferences } from './listening-engine'
...
export default function ListeningPanel({ sentences, currentDay, stopSignal, onPlaybackChange, cancelDictation }: Props) {
  const [preferences, setPreferences] = useState<ListeningPreferences>(() => {
    ...
    const parsed = parseListeningPreferences(stored)
    return stored === null ? { ...parsed, selectedDays: [currentDay] } : parsed
  })
  ...
  useEffect(() => {
    if (!isPlaying) { void wakeLock.current?.release(); wakeLock.current = null; return }
    const request = async () => {
      try {
        const sentinel = await navigator.wakeLock?.request('screen')
        if (!sentinel) return
        wakeLock.current = sentinel
      } catch { /* Wake Lock is an optional background enhancement. */ }
    }
    void request()
  }, [isPlaying])

  function update(next: Partial<ListeningPreferences>) {
    controller?.updatePreferences(next)
    setPreferences((value) => ({ ...value, ...next }))
  }
  function selectQuick(days: number[]) { update({ selectedDays: [...new Set(days.filter((day) => day >= 1 && day <= MAX_COURSE_DAY))].sort((a, b) => a - b), position: 0 }) }
  function start() {
  ...
  return <section className={`listening-panel ${preferences.drivingMode ? 'driving-mode' : ''}`} aria-labelledby="listening-heading">
    ...
    {item ? <article className="now-playing" aria-live="polite" aria-label="현재 재생 문장"><p className="now-korean" lang="ko">{item.korean}</p><p className="now-english" lang="en">{item.english}</p></article> : <p className="empty-state">재생 목록 설정에서 들을 Day를 선택하세요.</p>}
    <p className="now-playing-status" role="status"><strong>{label()}</strong> · {item ? `Day ${item.day} · ${playback.index + 1} / ${playlist.length}` : `0 / ${playlist.length}`}{playback.message && <> · {playback.message}</>}</p>
    ...
    <details className="listening-setup"><summary>재생 목록 설정 · {preferences.selectedDays.length ? `Day ${preferences.selectedDays.join(', ')}` : 'Day를 선택하세요'}</summary><p className="safety-copy"><strong>출발 전에 Day와 속도를 설정하세요.</strong> 운전 중에는 화면을 조작하지 말고, 화면을 계속 볼 필요가 없도록 재생을 준비해 주세요.</p><fieldset className="listening-days"><legend>들을 Day 선택</legend><div className="quick-actions"><button type="button" onClick={() => selectQuick([currentDay])}>현재 Day</button><button type="button" onClick={() => selectQuick(Array.from({ length: 3 }, (_, i) => currentDay + i))}>현재 Day부터 3일</button><button type="button" onClick={() => selectQuick(Array.from({ length: 7 }, (_, i) => currentDay + i))}>7일</button><button type="button" onClick={() => selectQuick(Array.from({ length: MAX_COURSE_DAY }, (_, i) => i + 1))}>전체 선택</button><button type="button" onClick={() => selectQuick([])}>전체 해제</button></div><div className="day-checkboxes">{Array.from({ length: MAX_COURSE_DAY }, (_, i) => i + 1).map((day) => <label key={day}><input type="checkbox" checked={preferences.selectedDays.includes(day)} onChange={(event) => selectQuick(event.target.checked ? [...preferences.selectedDays, day] : preferences.selectedDays.filter((value) => value !== day))} /> Day {day}</label>)}</div></fieldset><p className="selection-summary">선택: {preferences.selectedDays.length ? preferences.selectedDays.map((day) => `Day ${day}`).join(', ') : '없음'} · 기본 문장 {playlist.length}개</p><label className="filter"><input type="checkbox" checked={preferences.includeCustom} onChange={(event) => update({ includeCustom: event.target.checked })} /> 내 문장도 포함</label><div className="listening-settings"><label>한국어 속도 <input aria-label="한국어 속도" type="range" min={LISTENING_RATE_MIN} max={LISTENING_RATE_MAX} step="0.05" value={preferences.koreanRate} onChange={(event) => update({ koreanRate: Number(event.target.value) })} /><output>{preferences.koreanRate.toFixed(2)}배</output></label><label>영어 속도 <input aria-label="영어 속도" type="range" min={LISTENING_RATE_MIN} max={LISTENING_RATE_MAX} step="0.05" value={preferences.englishRate} onChange={(event) => update({ englishRate: Number(event.target.value) })} /><output>{preferences.englishRate.toFixed(2)}배</output></label><label>문장 사이 간격 <select aria-label="문장 사이 간격" value={preferences.pauseMs} onChange={(event) => update({ pauseMs: Number(event.target.value) })}><option value={300}>짧게 (0.3초)</option><option value={600}>보통 (0.6초)</option><option value={1000}>길게 (1초)</option><option value={1500}>아주 길게 (1.5초)</option></select></label></div></details>
