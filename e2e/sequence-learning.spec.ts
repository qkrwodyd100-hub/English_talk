...
test('keeps answers out of the pre-submit DOM and reveals declared alternatives only after checking', async ({ page }) => {
  await page.goto('/')
  await page.getByLabel('학습 Day 선택').selectOption('5')

  await expect(page.getByText('Can I book an Uber here?')).toHaveCount(0)
  await expect(page.getByText('Can I book a Lyft here?')).toHaveCount(0)
  await expect(page.getByText('저장된 표현과 슬롯')).toHaveCount(0)
  await expect(page.locator('.practice-prompt span')).toHaveText('택시와 차량 호출')
  await expect(page.getByText(/Beginner|우선순위/i)).toHaveCount(0)

  await page.getByRole('textbox', { name: '영어 답변' }).fill('Can I book a Lyft here?')
  await page.getByRole('button', { name: '정답 확인' }).click()
  await expect(page.getByText('Can I book an Uber here?')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Can I book a Lyft here?' })).toBeVisible()
