...
test('the 390px learning dashboard remains usable without horizontal overflow', async ({ page }) => {
  ...
  await expect(page.getByText('75일 동안 매일 10문장씩 학습해요.')).toBeVisible()
  await expect(page.getByRole('button', { name: '타이핑 연습' })).toBeVisible()
  expect(await page.locator('.study-tabs button').allTextContents()).toEqual(['타이핑 연습', '플래시카드', '듣기 학습', '오답 복습 (0)', '학습 기록', '학습 노트', '내 문장'])
  await expect(page.getByRole('heading', { name: /1 \/ 10/ })).toBeVisible()
  await expect(page.getByLabel('학습 현황')).toContainText('750')
